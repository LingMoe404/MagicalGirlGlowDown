...
def find_helper_command() -> tuple[str, ...]:
    configured = os.getenv("NOLLIERGBIDLE_GIGABYTE_HELPER")
    if configured:
        return (configured,)

    package_dir = Path(__file__).resolve().parent
    packaged = package_dir / "gigabyte_helper" / "NollieRGBIdle.GigabyteHelper.exe"
    if packaged.exists():
        return (str(packaged),)
    ...
    project = (
        ...
        / "helper"
        / "NollieRGBIdle.GigabyteHelper"
        / "NollieRGBIdle.GigabyteHelper.csproj"
    )
    if project.exists():
