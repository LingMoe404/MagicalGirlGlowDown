...
每次非文档代码推送后，GitHub Actions 会自动生成：

* `MagicalGirlGlowDown-v<版本>-Portable.7z`：解压后直接运行。
* `MagicalGirlGlowDown-v<版本>-Setup.exe`：安装时请求一次管理员权限，并创建无需每次确认 UAC 的登录启动任务。

前往仓库的 [Actions](https://github.com/LingMoe404/MagicalGirlGlowDown/actions/workflows/autobuild.yml)
页面，打开最新一次成功的 `AutoBuild`，在 Artifacts 区域下载对应产物。
