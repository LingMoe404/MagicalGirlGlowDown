...
The application is a Python 3.12.10 project managed by `uv`. Development startup is:
...
`MagicalGirlGlowDown` monitors for `NollieRGB.exe`.
...
Application data is stored under `%LOCALAPPDATA%\MagicalGirlGlowDown`.
