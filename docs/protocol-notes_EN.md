...
[简体中文](protocol-notes.md)

Static source: `NollieRGB.exe`, build timestamp 2026-05-07, PyInstaller
Python 3.11.
...
The device table in `src/magical_girl_glow_down/discovery.py` includes only
current application-mode devices from the recovered catalog. Bootloaders and
legacy devices are intentionally excluded from writes.
